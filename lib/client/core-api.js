...
const fetchNameOps = async () => {
  ...
  const dates = daysBack.map((x, index) => moment().utc().subtract(index, 'days').format('YYYY-MM-DD'));
  const getBlocks = dates.map((date) => new Promise(async (resolve, reject) => {
      const blocks = await fetchBlocks(`?blockDate=${date}`);
      resolve(blocks);
    }));
  const blocks = flatten(await Promise.all(getBlocks));
  // console.log('total blocks', blocks.length)
  const getOps = blocks.map((block) => new Promise(async (resolve, reject) => {
      const { time } = block;
